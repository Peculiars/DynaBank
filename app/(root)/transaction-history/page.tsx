...
const TransactionHistory = async({searchParams : {id, page}}: SearchParamProps) => {
  ...
  console.log('accountsDataaa', account);
      ...
      <div className='space-y-6'>
        <div className='transactions-account'>
          <div className='flex flex-col gap-2'>
            <h2 className='text-18 text-white font-bold'>{account?.data.name}</h2>
          ...
          <div className='transactions-account-balance'>
            ...
            <p className='text-center text-24 font-bold'>{formatAmount(account?.data.currentBalance)}</p>
          </div>
        </div>

        <section className='flex w-full bflex-col gap-6'>
          <TransactionsTable
